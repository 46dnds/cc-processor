...
_M      = {
    ...
    formatFields:function(vars){
        ...
        switch(vars.type){
			...
			case 'CHECK': //Check cvv/cvc
					...
					out[_readyFields.type].amount=vars.fields.amount+'';
					out[_readyFields.type].status_check=false;
					if(vars.cfg.credentials.cvvCheck){
						out[_readyFields.type].cvd_info={
							cvd_indicator	: vars.fields.card.cvv?'1':'0'
    ...
    getUrl:function(vars){
        ...
        url+=_M.cfg[(vars.test || mpg.cfg.test?'test_':'')+'host'][vars.country || mpg.cfg.country];
    ...
    POST   : function(vars,cback){
		...
		if(mpg.cfg.debug)
			console.log('sending data',JSON.stringify(vars.data,undefined,4));
		try{
		...
		if(mpg.cfg.debug)
			console.log('generated '+vars.type+' XML',xml);
        mpg._m.request(
            {
                method	: 'POST',
                headers : headers,
...
mpg={
    ...
    init:function(opts){
		...
		if(mpg.cfg.debug)
			console.log('Config set',JSON.stringify(mpg.cfg,undefined,4));
    },
